...
<template>
  <div class="login">
    <div class="login-overflow">
      <div
        class="login-banner"
        @mousemove="moveBanner($event)"
        @mouseout="clearMoveBanner($event)"
      >
        <img
          src="@/assets/img/login-icon-01.png"
          alt=""
          :style="{ transform: loginIcon01 }"
        />
        <img
          src="@/assets/img/login-icon-03.png"
          alt=""
          :style="{ transform: loginIcon03 }"
        />
      </div>
...
export default {
  ...
  methods: {
    ...
    // 校验账号
    checkPhone() {
      this.imgCodeTime = validationPhone(this.loginForm.username)
        ? new Date()
        : "";
    },
    // 校验表单
